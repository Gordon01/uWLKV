...
uwlkv_offset init_uwlkv(uwlkv_offset size)
{
	uwlkv_nvram_interface interface;
	interface.read  = &mock_flash_read;
	interface.write = &mock_flash_write;
	interface.erase = &mock_flash_erase;

	if (size)
	{
		interface.size = size < FLASH_REGION_SIZE ? size : FLASH_REGION_SIZE;
...
uwlkv_offset erase_nvram(uwlkv_offset size)
{
	mock_nvram_init();

	return init_uwlkv(size);
}

TEST_CASE("Initialization", "[init]")
{
	auto ret = erase_nvram(100);
	CHECK(0 == ret);
	ret = init_uwlkv(0);
	CHECK((FLASH_REGION_SIZE / UWLKV_ENTRY_SIZE) == ret);
...
{
	const auto capacity = erase_nvram(0);
